...
i = 0
while i < result[0][0]:
    library_score_queue = sorted(library_score_queue, key=lambda x: -sum([book_scores[i] for i in library_books[x] if i not in already_sign_up]) * (library_data[x][2] / library_data[x][1]))
