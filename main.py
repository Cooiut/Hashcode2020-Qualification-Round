...
library_data = ([] for i in range(libraries))
...
for i in range(len(library_score_queue)):
    ...
    result[(i + 1) * 2] = list(library_data[library_score_queue[i]][1])

    # 去重
    already_sign_up = already_sign_up | library_data[library_score_queue[i]][1]
